<form class="flex flex-col gap-4 pb-14 pt-3" action="{{ route('dashboard.konten.action') }}" method="POST" enctype="multipart/form-data">
    @csrf
    {{-- @method('post') --}}
    <div class="mb-3 flex gap-7">
        ...
        <div class="flex flex-col w-full gap-3">
            <input type="text" id="judul" name="judul"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value="{{ $isi[0]['judul'] }}"/>
            @error('judul')
    ...
    <div class="mb-3 flex gap-7">
        ...
        <div class="text-right">:</div>
        <div class="flex flex-col w-full gap-3">
            <textarea name="isi" id="isi" class="isi">{{ $isi[0]['isi'] }}</textarea>
            @error('isi')
                <div class="text-red-600 -mt-3 md:text-base text-sm">{{ $message }}</div>
            @enderror
        </div>
    ...
    <div class="mb-3 flex gap-7">
        ...
        <div class="text-right">:</div>
        <div class="flex flex-col w-full gap-3">
            <img class=" block w-72" src="../{{ $isi[0]['gambar'] }}" alt="profile">
